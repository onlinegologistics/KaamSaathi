...
const { upsertProfileSchema, accountTypeRequestSchema } = require('../validators/user.validator');
...
router.get('/:id/profile', requireAuth, validate(userIdSchema), userController.getPublicProfile);

router.post(
  '/account-type/request',
  requireAuth,
  validate(accountTypeRequestSchema),
  userController.requestAccountTypeChange
);
router.delete('/account-type/request', requireAuth, userController.cancelAccountTypeChangeRequest);
