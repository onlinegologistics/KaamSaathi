...
import { Banknote, CheckCircle2, Eye, ShieldCheck, Wallet, XCircle } from 'lucide-react';
...
export const ApprovalsPage = () => {
  ...
  const users = useMemo(
    () => mergeUsers(kycPending.data?.data, walletPending.data?.data),
    [kycPending.data?.data, walletPending.data?.data]
  );
  const isLoading = kycPending.isLoading || walletPending.isLoading;
  const actionPending = approveKyc.isPending || rejectKyc.isPending || approveWallet.isPending || rejectWallet.isPending;
  ...
  return (
    <div>
      <PageHeader
        title="Approvals"
        description="Review submitted KYC and wallet setup requests."
      />
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="p-8 text-center text-sm text-muted-foreground">
            No KYC or wallet approvals are pending.
          </CardContent>
        </Card>
      )}
